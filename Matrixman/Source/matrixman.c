...
struct Matrix read_file(char *dir_inputs, char *file_name)
{
    ...
    char *path_of_file = malloc(sizeof(char) * (strlen(dir_inputs) + strlen(file_name) + 2));
    snprintf(path_of_file, strlen(dir_inputs) + strlen(file_name) + 2, "%s/%s", dir_inputs, file_name);
...
int find_max(int *matrix, int matrix_size) 
{
    // if matrix_size = 0 means whole array has been traversed
    if (matrix_size == 1)
        return matrix[0];
    return max(matrix[matrix_size - 1], find_max(matrix, matrix_size - 1));
}

int find_min(int *matrix, int matrix_size) 
{
    // if matrix_size = 0 means whole array has been traversed
    if (matrix_size == 1)
        return matrix[0];
    return min(matrix[matrix_size - 1], find_min(matrix, matrix_size - 1));
}
...
struct Matrix create_zeros(char *name, int row_size, int col_size) // tested
{
    ...
    int i;
    int j;
    for (i = 0; i < matzeros.row_size; i++)
        for (j = 0; j < matzeros.col_size; j++)
            matzeros.rows[i][j] = 0;
    return matzeros;
}
...
struct Matrix pad_matrix(struct Matrix matrix, int x, int y, char *mode) // tested
{
    ...
    for (i = matrix.row_size; i < matrix.row_size + x; i++)
        for (j = 0; j < matrix.col_size; j++)
            padded_matrix.rows[i][j] = col_values[j];

    // padding towards to right of the matrix
    for (j = matrix.col_size; j < matrix.col_size + y; j++)
        for (i = 0; i < matrix.row_size; i++)
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < commands_size; i++)
    {
        for (j = 0; j < commands[i].token_size; j++)
